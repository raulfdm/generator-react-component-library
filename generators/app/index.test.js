...
describe('React Component Library Generator', () => {
  ...
  describe('Add Enzyme as tester', () => {
    ...
    it('copy all files from jest folder', () => {
      const babel = generateDistPath('babel.config.js');
      const jest = generateDistPath('jest.config.js');
      const setupTest = generateDistPath('./config/setupTest.js');
      const buttonTest = generateDistPath('./src/Button/Button.test.js');

      assert.file([babel, setupTest, jest, buttonTest]);
    });
  });
});
